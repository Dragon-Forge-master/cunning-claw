...
- "Done" is a claim about evidence, not intent. Report an action as completed ONLY when a tool result in THIS turn proves it: send_chat's "visible in the thread" line, a send_email confirmation, a file's bytes on disk, an exit 0 with the expected output. A keypress dispatching is not a message sent; a command running is not a job finished. Saying "sent" or "done" without that proof is the worst failure this house knows — worse than failing, because failing honestly gets fixed and false success gets discovered by ${config.persona.userName} at the worst possible moment. When you cannot verify, say exactly that: "I attempted it; here is what I can and cannot confirm."
...
- The operator may speak from the HUD, from Telegram or from Discord. Same person. Same approval rules.
...
export function resetHistory(): void {
  // Archive before wiping — always. Learned the hard way: a film-rig script
  // reset the conversation while the operator was mid-session with the claw,
  // and the context was simply gone. The journal kept the words, but the
  // working state deserved a copy too. A reset should never be able to
  // destroy; data/history-archive/ holds every cleared conversation.
  try {
    if (history.length > 0) {
      const dir = path.join(DATA_DIR, "history-archive");
      fs.mkdirSync(dir, { recursive: true });
      const stamp = new Date().toISOString().replace(/[:.]/g, "-");
      fs.writeFileSync(path.join(dir, `reset-${stamp}.json`), JSON.stringify(history, null, 2));
    }
  } catch { /* an archive failure must never block the reset itself */ }
  history = [];
  saveHistory(history);
