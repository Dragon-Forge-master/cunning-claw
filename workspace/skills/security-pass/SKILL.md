...
label: Security
category: craft
description: Review a change or a machine the way JARVIS is itself reviewed — denylist, secrets, untrusted input, approval. Use when asked if something is safe, to audit a diff, or before going public.
---
...
5. **Auth.** Loopback is not a permission boundary. The HUD cookie and `JARVIS_TOKEN` are. Say if an endpoint is missing them.
